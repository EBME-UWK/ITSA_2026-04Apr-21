...
**Update 22-Jun-2026**. Based on the reviewers comments we received on 22 June 2026, i.e.,

  -   Day-of-week effects: Daily data over four years will exhibit strong within-week perio-dicity (weekday vs. weekend admission volumes differ substantially). This is not mod-elled. Day-of-week fixed effects should be included, or the authors must justify their omission.
  
  -   Public holidays and the Christmas/New Year period: The interruption point (January 2024) coincides with a known seasonal peak in ED presentations (cited references 29 and 30). The influenza adjustment does not capture post-holiday effects. A holiday dummy should be considered.
  
  I have rerun the ITS regression analysis accounting for holiday and day of the week. To this end, Alexander Braun has sent me an Excel sheet `Feiertage_AT_Taggenau_2022_2025.xlsx`.
  
  $$ y_t = \beta_0 + \beta_1 t+\beta_2I_t+\beta_3(t-T)I_t + \beta_4F_t+ \beta_5H_t+\gamma_kWD_{k_t}+\varepsilon_t,$$
  where
  ...
  -   $F_{t}$ is an indicator variable (whether the day is during influenza period (1=yes, 0=no))
  -   $H_{t}$ is an indicator variable (whether the day is a public holiday in Austria (1=yes, 0=no))
  -   $WD_{k_{t}}$ are six indicator variables $k=2,...,7$ (whether the day is a particular day of the week (Tuesday,...,Sunday; yes=1, no=0).
...
holidays <- read_xlsx(str_c(filename,"Feiertage_AT_Taggenau_2022_2025.xlsx")) |>
            mutate(Triage_Begin_Date=as_date(dmy(Datum)),
                   Holiday=ifelse(Feiertag==TRUE,1,0)) |>
            dplyr::select(Triage_Begin_Date,Holiday)
...
dat00 <- read_xlsx(str_c(filename,"MTS Daten_2022-2025_complete.xlsx"),
         ...
         distinct() |>
           mutate(WD = recode(Week_Day,"Mo" = "Mon",
                  ...
                  WD2 = ifelse(WD=="Tue",1,0),
                  WD3 = ifelse(WD=="Wed",1,0),
                  WD4 = ifelse(WD=="Thu",1,0),
                  WD5 = ifelse(WD=="Fri",1,0),
                  WD6 = ifelse(WD=="Sat",1,0),
                  WD7 = ifelse(WD=="Sun",1,0)) |>
          left_join(holidays,by="Triage_Begin_Date")
...
##### Add analyses for reviewer comments from 22-Jun-2026 #####
datnew <- dat00 |> 
          group_by(Triage_Begin_Date,Day,Month,Year,Week_Day,Holiday,WD2,WD3,WD4,WD5,WD6,WD7) |>
          count() |> ungroup() |> addVariables()
```
...
getITSresultsNew <- function(dat0,outcome,ptitle,intday="01Jan2024"){
  form <- formula(paste0(outcome, " ~ Triage_Begin_Day + Level_Change + Slope_Change + influenza + Holiday + WD2 + WD3 + WD4 + WD5 + WD6 + WD7",sep=""))
  print(form)
  REML_model <- gls(model=form,
                    method="REML",
                    corr=corAR1(form=~Triage_Begin_Day),
  ...
  model_slope_change <- REML_model$coefficients[4]
  model_influenza <- REML_model$coefficients[5]
  model_holiday <- REML_model$coefficients[6]
  model_WD2 <- REML_model$coefficients[7]
  model_WD3 <- REML_model$coefficients[8]
  model_WD4 <- REML_model$coefficients[9]
  model_WD5 <- REML_model$coefficients[10]
  model_WD6 <- REML_model$coefficients[11]
  model_WD7 <- REML_model$coefficients[12]
  ...
  # and therefore is simple to generate by just using the intercept and pre_slope data
  dat0$counterfactual <- model_intercept + model_pre_slope*dat0$Triage_Begin_Day + 
                         model_influenza*dat0$influenza + model_holiday*dat0$Holiday + 
                         model_WD2*dat0$WD2 +
                         model_WD3*dat0$WD3 +
                         model_WD4*dat0$WD4 +
                         model_WD5*dat0$WD5 +
...
# How do I perform the ITSA accounting for influenza, holidays, and day of the week?

```{r}
#| echo: TRUE
getITSresultsNew(dat0=datnew,outcome="n",ptitle="All data")
```
...
getnts(dat0=datop,ptitle="During PHCC opening hours only")
getnts(dat0=datopn,ptitle="Outside PHCC opening hours only")
```
